...
2. Монструозное добавление. Я добавил два модуля и их механизм, удалил токены статичные. ЭТО У НАС Pathfinder Tokens: Character Gallery и Pathfinder Tokens: Myth and Magic. Постарался сделать полную локализацию, пришлось в коде покопаться, вроде получилось, за перевод не могу ручаться, так как я профан в английском(хе-хе), так что, прошу писать ошибки перевода в посте или Discord.

3. Теперь модуль весит под 1 гигабайт, и будет только расширяться, возможно разделю потом на два разных, один это чисто на компендиумы, то есть основной, а второй будет состоять из таких модулей как Character Gallery и Myth and Magic. Некоторые арты могут повторяться и быть разными, так как модули еще добавляли в сам компендиум, чистить буду позже, чтобы не было дублирования, уже код отредактирован, осталось только в ручную проверять.
